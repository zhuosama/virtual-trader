...
class MarketAnalystAgent:
    ...
    def _init_llm(self):
        ...
        try:
            ...
            logger.info("LLM 客户端初始化成功 (market_analyst)")
        except Exception as e:
            logger.warning(f"LLM 初始化失败，使用纯规则模式: {e}")
            self.llm = None

    def _llm_enhance_analysis(self, data_summary: str) -> str:
        """用 LLM 增强市场分析"""
        if not self.llm:
            return ""
        system = "你是A股市场分析专家。基于提供的数据，给出简洁的市场洞察（3-5句话）。关注趋势、风险和机会。用中文。"
        return self.llm.call("market_analyst", system, data_summary)
    ...
    def run_analysis(self) -> Dict:
        ...
        try:
            ...
            # 7. LLM 增强分析
            data_summary = f"市场状态: {market_regime}\n板块排名: {sector_ranking}\n风险信号: {risk_signals}\n指数: {indices}"
            llm_insight = self._llm_enhance_analysis(data_summary)
