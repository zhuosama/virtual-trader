...
class ReviewAgent:
    ...
    def _llm_deep_review(self, performance: str, trades: str, mistakes: str) -> str:
        """用 LLM 进行深度复盘"""
        if not self.llm:
            return ""
        system = ("你是A股交易复盘专家。基于今日绩效、交易记录和错误识别，进行深度反思。"
                  "找出：1)被忽视的市场信号 2)决策盲点 3)可改进的具体行为。用中文，每点一句话。")
        prompt = f"绩效:\n{performance}\n\n交易:\n{trades}\n\n错误:\n{mistakes}"
        return self.llm.call("review_agent", system, prompt)
